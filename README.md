...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  ...
  end-to-end with a simulated post URL). If a generated post has an image, the publisher converts
  the stored SVG/image data to PNG, uploads it as LinkedIn feed image media, and publishes the share
  with `shareMediaCategory: IMAGE`. Publishing a post also marks its topic and calendar entry as
  published.
