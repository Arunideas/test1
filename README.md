...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  ...
  end-to-end with a simulated post URL). When a generated image is attached, the LinkedIn client
  converts it to PNG, uploads it as a feed image asset, and publishes with `shareMediaCategory:
  IMAGE`. Publishing a post also marks its topic and calendar entry as published.
