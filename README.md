...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API; attached generated images are converted to PNG and uploaded as
  LinkedIn feed image media. Without credentials, the entire flow runs in safe simulation mode
  (jobs progress end-to-end with a simulated post URL). Publishing a post also marks its topic and
  calendar entry as published.
