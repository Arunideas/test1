...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. Posts with generated images register a LinkedIn image asset, upload a
  PNG version of the image, and publish with `shareMediaCategory: IMAGE`; otherwise the entire flow
  runs in safe simulation mode (jobs progress end-to-end with a simulated post URL). Publishing a
  post also marks its topic and calendar entry as published.
...
- **Works without a key** — image generation (local SVG renderer; upgrades to OpenAI images when a
  ...
  simulation, including image attachment checks), and community scaffolds (poll options, quiz Q&A,
