...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  ...
  end-to-end with a simulated post URL). If the post has a stored image, the publisher uploads it to
  LinkedIn and attaches it to the share. Publishing a post also marks its topic and calendar entry as
  published.
