...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  ...
  end-to-end with a simulated post URL). When a post has a generated image, publishing converts the
  stored SVG/data image to PNG and attaches it as LinkedIn image media. Publishing a post also marks
