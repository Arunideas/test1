...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. If the post has a generated image, it is converted to PNG, uploaded as
  a LinkedIn feed image, and attached to the share. Otherwise the entire flow runs in safe simulation
  mode (jobs progress end-to-end with a simulated post URL and note whether an image would be
  attached). Publishing a post also marks its topic and calendar entry as published.
