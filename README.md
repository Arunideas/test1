...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. When a post has a generated image, the publisher uploads it as a
  LinkedIn feed image and attaches it to the share. Otherwise the entire flow runs in safe
