...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API; if a generated post has an `imageId`, the stored SVG/data image is
  normalized to PNG and attached as LinkedIn image media. Otherwise the entire flow runs in safe
  simulation mode (jobs progress end-to-end with a simulated post URL). Publishing a post also
  marks its topic and calendar entry as published.
...
4. **Create the stack in Portainer:** *Stacks → Add stack*. Paste `docker-compose.yml` (or point it
   ...
   and `CRON_SECRET`. Deploy. `sharp` is included for server-side SVG/data-image to PNG conversion
   before LinkedIn upload.
