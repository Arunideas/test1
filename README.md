...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API; generated images are converted to LinkedIn-ready PNGs and attached
  as feed media. Without those credentials, the entire flow runs in safe simulation mode (jobs
  progress end-to-end with a simulated post URL). Publishing a post also marks its topic and
