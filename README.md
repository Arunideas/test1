...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. Posts include their generated image: OpenAI images upload directly,
  and locally-rendered SVG cards are converted to PNG before upload. Without credentials, the
  entire flow runs in safe simulation mode (jobs progress end-to-end with a simulated post URL).
