...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. Attached SVG images are converted to PNG, uploaded as feed images,
  and published with `shareMediaCategory: IMAGE`; otherwise the entire flow runs in safe simulation
  mode (jobs progress end-to-end with a simulated post URL). Publishing a post also marks its topic
  and calendar entry as published.
