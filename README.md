...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. Generated images are uploaded and attached to the post; local SVG
  image cards are converted to LinkedIn-ready PNGs automatically. Without credentials, the entire
  flow runs in safe simulation mode (jobs progress end-to-end with a simulated post URL). Publishing
