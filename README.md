...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  ...
  end-to-end with a simulated post URL). When a post has a generated image, the publisher converts
  it to PNG with `sharp`, uploads it to LinkedIn as feed image media, and publishes the share with
  `shareMediaCategory: IMAGE`. Publishing a post also marks its topic and calendar entry as
  published.
...
- Publishing: `POST /api/publish`, `GET /api/publish/jobs`, `POST /api/publish/jobs/[id]`
  ...
  `POST /api/publish/auto-schedule`. LinkedIn image posts require `sharp` so SVG/data-URI images
  can be uploaded as PNG bytes.
