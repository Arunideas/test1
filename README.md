...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API. Posts with saved images register and upload a PNG asset before the
  UGC post is created; otherwise the entire flow runs in safe simulation mode (jobs progress
