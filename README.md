...
- **Real or simulated** — set `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_AUTHOR_URN` to post for real via
  the LinkedIn UGC Posts API; when a generated image is attached, the publisher registers a LinkedIn
  feed image upload, converts the visual to PNG, and publishes with `shareMediaCategory: IMAGE`.
  Without LinkedIn credentials, the entire flow runs in safe simulation mode (jobs progress end-to-end
